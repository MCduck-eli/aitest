...
const initializeDatabase = async () => {
    ...
    try {
        ...
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_users_training_center ON users(training_center_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_test_banks_training_center ON test_banks(training_center_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_questions_test_bank ON questions(test_bank_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_exam_results_test_bank ON exam_results(test_bank_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_exam_results_student ON exam_results(student_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_violations_exam_result ON proctoring_violations(exam_result_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_subjects_training_center ON subjects(training_center_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_study_groups_training_center ON study_groups(training_center_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_lesson_scripts_training_center ON lesson_scripts(training_center_id);`);
        await (0, database_1.query)(`CREATE INDEX IF NOT EXISTS idx_student_progress_training_center ON student_progress(training_center_id);`);
        ...
        const masterAdminPasswordHash = await (0, auth_1.hashPassword)(masterAdminPassword);
        await (0, database_1.query)(`INSERT INTO users (email, password_hash, full_name, role, is_active)
             VALUES ($1::varchar, $2::varchar, $3::varchar, 'super_admin', true)
             ON CONFLICT (email) DO UPDATE SET
                 password_hash = EXCLUDED.password_hash,
                 full_name = EXCLUDED.full_name,
                 role = EXCLUDED.role,
                 is_active = EXCLUDED.is_active;`, [masterAdminEmail, masterAdminPasswordHash, masterAdminName]);
        // Clean up 'Main Training Center' if exists (not needed anymore)
        await (0, database_1.query)(`DELETE FROM training_centers WHERE name = 'Main Training Center'`);
